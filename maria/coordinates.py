...
class Coordinates:
    ...
    def __init__(
        ...
        _points = hp.ang2vec(np.pi / 2 - _theta.T, _phi.T)

        _center_cotheta, _center_phi = hp.vec2ang(_points.mean(axis=0))
        _center_theta = np.pi / 2 - _center_cotheta
        ...
        DS_FID_POINTS = np.swapaxes(
            hp.ang2vec(np.pi / 2 - DS_FID_THETA.T, DS_FID_PHI.T), -2, -1
        )
        ...
        for new_frame in ["ra_dec", "az_el", "galactic"]:
            ...
            DS_FID_POINTS_NEW_FRAME = np.swapaxes(
                hp.ang2vec(
                    np.pi / 2 - DS_FID_THETA_NEW_FRAME.T, DS_FID_PHI_NEW_FRAME.T
                ),
                -2,
                -1,
            )
