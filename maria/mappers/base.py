...
from ..map import VALID_MAP_QUANTITIES, Map, ProjectionMap
from ..tod import TOD, VALID_TOD_QUANTITIES
...
class BaseMapper:
    ...
    def __init__(
        ...
        if stokes is None:
            ...
            logger.info(f"Inferring stokes parameters '{self.stokes}' for mapper from detector sensitivities")
...
class BaseProjectionMapper(BaseMapper):
    ...
    def __init__(
        ...
        if resolution is None:
            resolution = Quantity(np.min([tod.dets.fwhm.deg.min() for tod in tods]) / 2, "deg")
            logger.info(f"Inferring resolution = {resolution} from detector FWHM")
        ...
        if center is None:
            ...
            logger.info(f"Inferring center {repr_phi_theta(phi=center[0].rad, theta=center[1].rad, frame=frame)} for mapper")
    ...
    def map(self):
        ...
        m = ProjectionMap(
            ...
            t=self.t,
            nu=self.nu,
            xi_res=self.resolution,
            eta_res=-self.resolution,
            center=self.center,
            degrees=False,
