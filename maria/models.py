...
class LinearAngularModel(AtmosphericModel):
    ...
    def __init__(self, array, pointing, site, config=DEFAULT_LAM_CONFIG, verbose=False):
        ...
        self.waists = self.array.get_beam_waist(self.layer_depths[:,None], self.array.primary_size, self.spectrum.nu)
        ...
        self.weather = weathergen.Weather(region=self.site.region, time=self.pointing.unix, mode='median', generate=True)
        ...
        for attr in ['water_vapor', 'temperature', 'pressure', 'wind_north', 'wind_east']:
            ...
            setattr(self, attr, sp.interpolate.RegularGridInterpolator((self.weather.height, self.weather.time), 
                                                    getattr(self.weather, attr))((self.heights, self.pointing.unix[None])))
        ...
        self.AWV_X = (+ self.wind_east * np.cos(self.pointing.az[None]) - self.wind_north * np.sin(self.pointing.az[None])) / self.layer_depths[:,None]
        self.AWV_Y = (- self.wind_east * np.sin(self.pointing.az[None]) + self.wind_north * np.cos(self.pointing.az[None])) / self.layer_depths[:,None] * np.sin(self.pointing.el[None])
        
        self.REL_X = self.array.offset_x[None, :, None] + self.pointing.dx[None, None] + np.cumsum(self.AWV_X * self.pointing.dt, axis=-1)[:, None]
        self.REL_Y = self.array.offset_y[None, :, None] + self.pointing.dy[None, None] + np.cumsum(self.AWV_Y * self.pointing.dt, axis=-1)[:, None] 
        ...
        for i_l, depth in enumerate(self.layer_depths):
            ...
            MEAN_REL_POINTS += 1e-12 * np.random.standard_normal(size=MEAN_REL_POINTS.shape) # some jitter for the hull
        ...
        with tqdm(total=len(self.layer_depths),desc='Computing weights') as prog:
            for i_l, (depth, LX, LY, AR, GZ) in enumerate(zip(self.layer_depths,self.X,self.Y,self.AR_samples,self.genz)):
                ...
                C00 = utils._approximate_normalized_matern(R00, self.outer_scale/depth, 5/6) + alpha * np.eye(len(X0))
                C01 = utils._approximate_normalized_matern(R01, self.outer_scale/depth, 5/6) 
                C11 = utils._approximate_normalized_matern(R11, self.outer_scale/depth, 5/6) + alpha * np.eye(len(X1))
